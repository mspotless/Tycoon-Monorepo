...
describe('Game Exceptions', () => {
  ...
  describe('GameStatusTransitionException', () => {
    it('should create exception with status transition', () => {
      const exception = new GameStatusTransitionException(
        'RUNNING',
        'PENDING',
        123,
      );
      ...
      expect(exception.message).toBe(
        'Invalid game status transition from RUNNING to PENDING',
      );
      expect(exception.details).toEqual({
  ...
  describe('mapValidationErrorToGameException', () => {
    it('should map enum validation error', () => {
      ...
      expect(exception.message).toBe(
        'Invalid mode: mode must be either PUBLIC or PRIVATE',
      );
      expect(exception.details).toEqual({
        field: 'mode',
        value: 'INVALID',
    ...
    it('should map integer validation error', () => {
      ...
      expect(exception.errorCode).toBe('GAME_VALIDATION_ERROR');
      expect(exception.message).toBe(
        'Invalid numberOfPlayers: Must be an integer',
      );
    });
    ...
    it('should map min validation error', () => {
      ...
      expect(exception.errorCode).toBe('GAME_VALIDATION_ERROR');
      expect(exception.message).toBe(
        'Invalid numberOfPlayers: Value is too small',
      );
    });
    ...
    it('should map max validation error', () => {
      ...
      expect(exception.errorCode).toBe('GAME_VALIDATION_ERROR');
      expect(exception.message).toBe(
        'Invalid numberOfPlayers: Value is too large',
      );
    });
    ...
    it('should map date string validation error', () => {
      ...
      expect(exception.errorCode).toBe('GAME_VALIDATION_ERROR');
      expect(exception.message).toBe(
        'Invalid startTime: Must be a valid date string',
      );
    });
      ...
      const errors = [
        {
          ...
          constraints: {
            isGameCode:
              'Game code must be exactly 6 characters containing only uppercase letters and numbers',
          },
        },
      ];
    ...
    it('should map player count validation error', () => {
      ...
      expect(exception.errorCode).toBe('GAME_VALIDATION_ERROR');
      expect(exception.message).toBe(
        'Invalid numberOfPlayers: Invalid player count',
      );
    });
    ...
    it('should map blockchain address validation error', () => {
      ...
      expect(exception.errorCode).toBe('GAME_VALIDATION_ERROR');
      expect(exception.message).toBe(
        'Invalid address: Invalid blockchain address',
      );
    });
    ...
    it('should map contract game ID validation error', () => {
      ...
      expect(exception.errorCode).toBe('GAME_VALIDATION_ERROR');
      expect(exception.message).toBe(
        'Invalid contract_game_id: Invalid contract game ID',
      );
    });
    ...
    it('should map game placement validation error', () => {
      ...
      expect(exception.errorCode).toBe('GAME_VALIDATION_ERROR');
      expect(exception.message).toBe(
        'Invalid placements: Invalid game placement format',
      );
    });
    ...
    it('should handle unknown validation error', () => {
      ...
      expect(exception.errorCode).toBe('GAME_VALIDATION_ERROR');
      expect(exception.message).toBe(
        'Invalid field: Unknown constraint message',
      );
    });
